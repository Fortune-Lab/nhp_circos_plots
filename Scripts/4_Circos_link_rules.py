#This script is meant to take a barcode input file, define barcodes shared across tissue and then create text describing these links (e.g., origin sites and color of ribbons) that will be pasted into the Circos configuration file.
...
#create dictionary of all potential links, including redundance and reciprocal links
...
#create a color rule for each unique band/link
...
for i in unique_sites:
	seg_ID=str(unique_links[i][0])+'_'+str(unique_links[i][1])+'_'+str(unique_links[i][2])
	if seg_ID not in segments:
		segments.append(seg_ID)
		print('<rule>')
		print('condition = on(',unique_links[i][0],')')
		print('condition = var(start1) >= ',unique_links[i][1])
		print('condition = var(end1) <= ',unique_links[i][2])
		colorname=unique_links[i][6]
		print('color = '+colorname+'_a4')
		print('radius2 = 1.0r')
		print('</rule>')
		print('#')
